...
export class PostsService {
  ...
  async createPost(req: Request, res: Response, next: NextFunction) {
    ...
    if (req.files) {
      for (const file of req.files as Express.Multer.File[]) {
        try {
          const cnr_res = await this.cloudinaryRepo.uploadFile(file.path, file.filename, "posts");
          images.push({
          ...
          return next(createError.InternalServerError(`Error uploading file: ${error}`));
        }
      }
    }
  ...
  async getPostsWithUserLikes(req: Request, res: Response, next: NextFunction) {
    try {
      if (!validateRequestErrors(req, next)) return;

      const payload = req.payload as IPayload;
      const posts = await this.postRepository.getPostsWithUserLikes(
        payload.userdocId
      );
  ...
  async getPosts(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const payload = req.payload as IPayload;
      const posts = await this.postRepository.getPosts(payload.userdocId);
      return SendApiResponse(res, 200, posts, "success");
    } catch (error) {
      next(createError.InternalServerError(`${error}`));
    }
